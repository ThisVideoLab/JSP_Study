...
<%@ include file = 'db_connection_oracle.jsp'%> <!--  오라클 데이터 서버에 접속하는 파일 불러오기 -->
...
<%
	...
	try{
		...
		if(rs.next()){ // DB에 해당 폼에서 넘긴 ID가 존재한다면 ==> 폼에서 넘긴 패스워드와 DB의 패스워드가 일치되는지 확인해야 함
			...
			if(id.equals(rs_id)&&pass.equals(rs_pass)){ // ID 존재 여부는 이미 확인했기 때문에, 비밀번호만 조건으로 체크함 
				//DB에서 가져온 패스워드와 폼에서 넘겨받은 PASS값이 일치할 경우에 업데이트를 실행함
				
				sql = "update mbTbl set name = '" + name + "', email = '" + email + "', city = '" + city + "', " + "phone = '" + phone + "' where id = '" + id + "'";
